...
const NATIVE_EMBED_PATH = join(ROOT, "src/native-addon-embed.generated.ts");
...
/** A build-unique identity baked into the binary: git sha (marked `-dirty.<epoch>`
 *  when the tree isn't clean, so uncommitted rebuilds still differ). The daemon
 *  reports this at boot and the CLI compares it to its own binary's value to
 *  detect a daemon left running on OLD code after an upgrade (see code-version.ts). */
function computeBuildId(): string {
  try {
    const sha = require("node:child_process")
      ...
      .toString()
      .trim();
    const dirty = require("node:child_process")
...
/** Generate version file with current version + build id */
...
function restoreCloudBootstrapStub() {
  const stub = `/**
 * STUB FILE - Replaced during build
 * Auto-generated by scripts/build.ts
 */
...
`;
  ...
  writeFileSync(CLOUD_BOOTSTRAP_BUNDLE_PATH, stub);
}

async function buildWeb() {
  console.log("📦 Building web app...");
  restoreStub(); // Ensure stub is in place before web build
  restoreCloudBootstrapStub();
  await $`cd ${join(ROOT, "web")} && bun install && bun run build`;
  console.log("✓ Web app built");
}

async function generateCloudBootstrapBundle() {
  console.log("🧩 Generating cloud bootstrap bundle...");

  if (!existsSync(CLOUD_BOOTSTRAP_ENTRY)) {
    throw new Error(`Missing cloud bootstrap entrypoint: ${CLOUD_BOOTSTRAP_ENTRY}`);
  }

  const result = await Bun.build({
    entrypoints: [CLOUD_BOOTSTRAP_ENTRY],
    target: 'bun',
    format: 'esm',
    minify: true,
    splitting: false,
...
`;
  ...
  writeFileSync(CLOUD_BOOTSTRAP_BUNDLE_PATH, code);
...
/**
 * Map a build target key to the pi-natives .node filename(s) for that platform.
 * arm64 targets have a single file; x64 targets have baseline + modern variants.
 * We embed the baseline variant for x64 since it works on all x64 CPUs.
 */
function getNativeAddonFilename(target: TargetKey): string {
  switch (target) {
    case "darwin-arm64": return "pi_natives.darwin-arm64.node";
    case "darwin-x64":   return "pi_natives.darwin-x64-baseline.node";
    case "linux-x64":    return "pi_natives.linux-x64-baseline.node";
    case "linux-arm64":  return "pi_natives.linux-arm64.node";
  }
}
 ...
 *
 * In dev mode (running via `bun src/index.ts`), the guard on import.meta.url
 * ensures this is a no-op — pi-natives loads from node_modules directly.
 */
function generateNativeAddonEmbed(target: TargetKey) {
  const filename = getNativeAddonFilename(target);
  const piNativesPkg = JSON.parse(readFileSync(
    join(ROOT, "node_modules/@oh-my-pi/pi-natives/package.json"), "utf-8"
  ));
  const version = piNativesPkg.version;

  const code = `// AUTO-GENERATED by scripts/build.ts \u2014 DO NOT EDIT
// Embeds the pi-natives .node addon for ${target} into the compiled binary.
...
`;
  ...
  writeFileSync(NATIVE_EMBED_PATH, code);
  console.log(`✓ Generated native addon embed for ${target} (${filename})`);
}
...
async function main() {
  ...
  await generateCloudBootstrapBundle();
