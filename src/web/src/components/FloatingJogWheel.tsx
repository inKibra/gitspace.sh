...
import { getDirectionAndDistance, type Direction } from './DPad';
...
export function FloatingJogWheel({ onDirection }: FloatingJogWheelProps): React.ReactElement {
  ...
  return (
    ...
    >
      {/* Outer ring */}
      <div
        className="absolute rounded-full border-2 border-[#585b70] bg-[#1e1e2e]/80"
        style={{
      ...
      {/* Label */}
      <div className="absolute text-[10px] text-[#6c7086] whitespace-nowrap" style={{ bottom: -16 }}>
        Arrows
      </div>
    </div>
  );
