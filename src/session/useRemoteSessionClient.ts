...
import type { ScriptRuntimeState, BackendSessionState, SessionEngineState } from './types.js';
import type {
  BundleRefreshPlan,
...
export interface UseRemoteSessionClientReturn<ConnectParams> {
  /** Session engine state for useWorkspaceCatalog and other shared workspace controllers */
  engineState: SessionEngineState | null;
...
export function useRemoteSessionClient<ConnectParams>(
  ...
  const setWorkspaceStatus = useCallback(async (
    projectName: string,
    workspaceName: string,
    phase: import('../types/config.js').WorkspacePhase
  ): Promise<void> => {
    const result = await withActiveBackend((backendKey) =>
      engine.setWorkspaceStatus(backendKey, projectName, workspaceName, phase)
    );
    if (result === null) {
      throw createMissingBackendError('setWorkspaceStatus');
    }
    requestWorkspaces();
  }, [engine, withActiveBackend, requestWorkspaces]);
  ...
  useEffect(() => {
    ...
    if (selectedProjectName && projects.some((project: { name: string }) => project.name === selectedProjectName)) {
      return;
    }

    const preferredProjectName = projects.find((project: { name: string; isCurrent: boolean }) => project.isCurrent)?.name ?? projects[0]?.name ?? null;
