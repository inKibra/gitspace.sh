...
export const lifecycleScripts: readonly LifecycleScript[] = [
  {
    id: 'machine/prepare/10-install.sh',
    phase: 'machine/prepare',
    path: 'machine/prepare/10-install.sh',
    command: '.gitspace/lifecycle/machine/prepare/10-install.sh',
  ...
  {
    id: 'workspace/materialize/20-db.sh',
    phase: 'workspace/materialize',
    path: 'workspace/materialize/20-db.sh',
    command: '.gitspace/lifecycle/workspace/materialize/20-db.sh --migrate-and-seed',
    trust: {
      status: 'changed',
      commandHash: 'sha256:be81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbca',
      approvedCommand: '.gitspace/lifecycle/workspace/materialize/20-db.sh --migrate',
      currentCommand: '.gitspace/lifecycle/workspace/materialize/20-db.sh --migrate-and-seed',
      approvedBy: 'Studio browser',
  ...
  {
    id: 'machine/prepare/30-xcode.ios.sh',
    phase: 'machine/prepare',
    path: 'machine/prepare/30-xcode.ios.sh',
    command: '.gitspace/lifecycle/machine/prepare/30-xcode.ios.sh',
    profiles: ['ios'],
    trust: approvedTrust('sha256:5611fb20', 'Aug 28, 2026'),
    lastRun: { status: 'never' },
  },
  {
    id: 'workspace/materialize/10-env.sh',
    phase: 'workspace/materialize',
    path: 'workspace/materialize/10-env.sh',
    command: '.gitspace/lifecycle/workspace/materialize/10-env.sh',
    trust: approvedTrust('sha256:8aa351cc', 'Aug 28, 2026'),
    lastRun: { status: 'succeeded', relativeTime: 'just now', duration: '0.4s', output: 'Workspace environment materialized.' },
  },
  {
    id: 'cloud/destroy/10-drop-preview.sh',
    phase: 'cloud/destroy',
    path: 'cloud/destroy/10-drop-preview.sh',
    command: '.gitspace/lifecycle/cloud/destroy/10-drop-preview.sh',
...
export const environmentFixture: EnvironmentViewModel = {
  ...
  ledger: {
    ...
    runs: [
      {
        id: 'materialize-2', projectId: 'gitspace', spaceId: 'agent-blame', phase: 'workspace/materialize', status: 'failed',
        profile: 'backend', machineId: 'local-machine', generation: 2,
        executionHashes: ['sha256:be81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbcabe81bbca'], terminalName: null,
        results: [{ id: 'workspace/materialize/20-db.sh', exitCode: 1, output: 'Migration 0006 failed: database connection refused.' }],
        output: 'Migration 0006 failed: database connection refused.\nProvisioned database and endpoint are unchanged.', exitCode: 1,
        startedAt: '2026-09-05T12:00:00.000Z', finishedAt: '2026-09-05T12:00:18.000Z',
        deadlineAt: '2026-09-05T12:10:00.000Z', cancelRequestedAt: null,
        failure: { code: 'ExecutionFailed', message: 'Migration 0006 failed: database connection refused.', context: { runId: 'materialize-2', exitCode: '1' } },
        incidents: [{
          id: 'materialize-2:failure', kind: 'domain', occurredAt: '2026-09-05T12:00:18.000Z',
          message: 'Migration 0006 failed: database connection refused.',
          failure: { code: 'ExecutionFailed', message: 'Migration 0006 failed: database connection refused.', context: { runId: 'materialize-2', exitCode: '1' } },
        }],
      },
      {
        id: 'provision-1', projectId: 'gitspace', spaceId: 'agent-blame', phase: 'cloud/provision', status: 'succeeded',
        profile: 'backend', machineId: 'studio', generation: null, executionHashes: [], terminalName: null,
        results: [], output: 'Created preview-agent-blame. Resource bindings saved.', exitCode: 0,
        startedAt: '2026-09-01T11:59:30.000Z', finishedAt: '2026-09-01T12:00:00.000Z',
        deadlineAt: '2026-09-01T12:09:30.000Z', cancelRequestedAt: null, failure: null, incidents: [],
      },
    ],
  },
  secrets: [
