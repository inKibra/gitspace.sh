...
import { withDeploymentSqliteContext } from './sqlite-diagnostics.js';
...
export class DeploymentEngine {
  ...
  async execute(plan: DeploymentPlan): Promise<ResultType<DeploymentRunRecord, DeploymentExecutionError>> {
    return withDeploymentSqliteContext(
      ...
      async () => {
        ...
        return withDeploymentSqliteContext({ attempt: run.attempt }, async () => {
          for (const [ordinal, artifact] of plan.artifacts.entries()) {
            withDeploymentSqliteContext({ target: artifact.entrypoint, phase: 'pending' }, () =>
          ...
          for (const phase of phases) {
            const failure = await withDeploymentSqliteContext({ phase: phase.action }, async () => {
            ...
            if (failure) return this.failAndRollback(plan, failure);
          }
  ...
  private async finalize(
    ...
    return withDeploymentSqliteContext(
      { deploymentId: plan.id, releaseSha: plan.source.revision, attempt: run.attempt, phase: 'finalize' },
      async () => {
  ...
  private async invoke(
    ...
    return withDeploymentSqliteContext(
      {
        deploymentId: context.plan.id,
        releaseSha: context.plan.source.revision,
        target: driver.entrypoint,
      ...
      async () => {
        try {
          ...
          const converted = new ReplacementActionError({
            entrypoint: driver.entrypoint,
            phase,
            message: error instanceof Error ? error.message : String(error),
          });
          // Keep the original stack and SQLite evidence reachable without changing wire errors.
          Object.defineProperty(converted, 'cause', { value: error, configurable: true });
  ...
  private async failAndRollback(
    ...
    return withDeploymentSqliteContext(
      { deploymentId: plan.id, releaseSha: plan.source.revision, phase: 'rollback' },
      async () => {
        this.journal.transition(plan.id, 'rolling-back', failure.message);
        const steps = this.journal
          .steps(plan.id)
          .filter((step) => needsRollback(step.state))
          .reverse();
  ...
  private async rollbackRecordedAttempt(
    ...
    return withDeploymentSqliteContext(
      { deploymentId: plan.id, releaseSha: plan.source.revision, attempt: run.attempt, phase: 'rollback' },
      async () => {
