...
describe('registered browser invitations', () => {
  ...
  it('denies foreign browsers, account host substitution and revoked issuers', async () => {
    const owner = await account();
    const other = { ...owner, userId: `u-${'0'.repeat(32)}`, handle: 'foreign-tenant', deviceId: crypto.randomUUID() };
    const invite = await register(owner);
    const foreign = await SELF.fetch(signedRequest({ ...other, handle: owner.handle }, 'status', { inviteId: invite.invite.inviteId }));
    expect(foreign.status).toBe(403);
    expect(await foreign.json()).toMatchObject({ error: { code: 'ACCOUNT_UNAVAILABLE' } });
  ...
  it('blocks invitation cancellation and redemption when the platform suspends the tenant', async () => {
    const owner = await account();
