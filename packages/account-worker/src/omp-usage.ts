...
async function fetchClaudePayload(url: string, headers: Record<string, string>, fetcher: typeof fetch, signal: AbortSignal): Promise<Json | null> {
  ...
  for (let attempt = 0; attempt < CLAUDE_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        failure = new Error(`Claude usage request failed with status ${response.status}`);
        retryable = response.status >= 500 || response.status === 408;
        retryAfter = response.headers.get('retry-after');
      }
    } catch (error) {
      failure = error;
      if (signal.aborted || (isRecord(error) && (error.name === 'AbortError' || error.name === 'TimeoutError'))) throw error;
    }
  ...
  if (!lastPayload && failure) throw failure;
  return lastPayload;
}
...
async function fetchClaudeUsage(credential: StoredOAuthCredential, fetcher: typeof fetch, signal: AbortSignal): Promise<UsageReport | null> {
  ...
  if (!payload) throw new Error('Claude usage returned an invalid response');
  const limits = claudeLimits(payload);
  if (limits.length === 0) throw new Error('Claude usage returned no usage limits');
...
async function fetchCodexUsage(credential: StoredOAuthCredential, fetcher: typeof fetch, signal: AbortSignal): Promise<UsageReport | null> {
  ...
  if (credential.expires <= nowMs) throw new Error('Codex usage credential expired; refresh the provider sign-in');
  ...
  if (!isRecord(payload)) throw new Error('Codex usage returned an invalid response');
  ...
  if (limits.length === 0) throw new Error('Codex usage returned no usage limits');
...
/**
 * Fetch usage for every credential with a supported provider. Keep partial
 * successes, but fail an entirely unavailable aggregate instead of caching a
 * successful empty response. Never return tokens or provider raw bodies.
 */
export async function fetchUsageReports(credentials: readonly StoredOAuthCredential[], fetcher: typeof fetch = fetch): Promise<UsageReport[]> {
  ...
  settled.forEach((result, index) => {
    ...
    if (result.status === 'rejected' || !result.value) {
      let reason = result.status === 'rejected' && result.reason instanceof Error
      ...
      failures.push(`${credential.provider}: ${reason}`);
      console.warn('omp usage fetch failed', { provider: credential.provider, error: reason });
      return;
    }
  ...
  if (reports.length === 0 && failures.length > 0) throw new Error(`Unable to fetch provider usage: ${failures.join('; ')}`);
  return reports;
}
