...
export async function ensureAccountGitSpaceProject(
  ...
  const [frontend, account] = await Promise.all([
    ...
    accountsNamespace.get(accountsNamespace.idFromName('account')).get(userId),
  ]);
  ...
  if (frontend) {
    ...
    if (object) {
      ...
      const status = await releasesNamespace.get(releasesNamespace.idFromName(userId)).status(userId, { sha: null, version: null });
