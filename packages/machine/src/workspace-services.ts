...
function serviceHostname(domain: string, namespace: string, spaceId: string, serviceName: string): string {
  const prefix = `${normalizeLabel(serviceName)}--${normalizeLabel(spaceId)}--${normalizeLabel(namespace)}-srv`;
  if (prefix.length <= 63) return `${prefix}.${domain}`;
  const digest = new Bun.CryptoHasher('sha256').update(prefix).digest('hex').slice(0, 10);
  return `${prefix.slice(0, 52).replace(/-+$/u, '')}-${digest}.${domain}`;
}
