...
export function createGitSpaceRpcRouter(options: GitSpaceRpcRouterOptions) {
  ...
  const getSessionControl = server.implement(getSessionControlContract).handler(async ({ input, errors }) => {
    if (!options.sessions.get(input.sessionId)) return err(errors.SessionNotFound({ sessionId: input.sessionId }));
    try { return ok(options.sessions.control(input.sessionId)); }
  ...
  const setSessionThinking = server.implement(setSessionThinkingContract).handler(async ({ input, errors }) => {
    if (!options.sessions.get(input.sessionId)) return err(errors.SessionNotFound({ sessionId: input.sessionId }));
    try { return ok(options.sessions.setThinking(input.sessionId, input.thinking)); }
  ...
  const setSessionFast = server.implement(setSessionFastContract).handler(async ({ input, errors }) => {
    if (!options.sessions.get(input.sessionId)) return err(errors.SessionNotFound({ sessionId: input.sessionId }));
    try { return ok(options.sessions.setFast(input.sessionId, input.enabled)); }
  ...
  const setSessionApproval = server.implement(setSessionApprovalContract).handler(async ({ input, errors }) => {
    if (!options.sessions.get(input.sessionId)) return err(errors.SessionNotFound({ sessionId: input.sessionId }));
    try { return ok(options.sessions.setApproval(input.sessionId, input.approvalMode)); }
  ...
  const clearSessionQueue = server.implement(clearSessionQueueContract).handler(async ({ input, errors }) => {
    if (!options.sessions.get(input.sessionId)) return err(errors.SessionNotFound({ sessionId: input.sessionId }));
    try { return ok(options.sessions.clearQueue(input.sessionId)); }
  ...
  // The wire view omits the worker upload metadata; the zod record carries it for the platform.
  ...
  // Routing table: every space this machine knows about (cloud-materialized,
  // so all of them) with its holder's endpoint from the fleet catalog.
  ...
  const placementView = (space: MaterializedSpace, endpoints: Record<string, string | null>): SpacePlacementView => ({
    spaceId: space.id,
    projectId: space.projectId,
    kind: space.kind === 'worktree' ? 'worktree' : 'base',
    holderId: space.holderId,
    state: space.placementState,
    endpoint: endpoints[space.holderId] ?? null,
  });
  const placements = server.implement(placementsContract).handler(async ({ errors }) => {
    try {
      const endpoints = await holderEndpoints();
      const spaces = options.database.listProjects().flatMap((project) => options.database.listSpaces(project.id)).map((space) => placementView(space, endpoints));
  ...
  const locateSession = server.implement(locateSessionContract).handler(async ({ input, errors }) => {
    try {
      ...
      const localSpace = local ? options.database.getSpace(local.spaceId) : null;
      if (localSpace) return ok(placementView(localSpace, endpoints));
      for (const project of options.database.listProjects()) {
        const canonical = (await options.canonicalSessions?.(project.id))?.find((session) => session.id === input.sessionId);
        const space = canonical ? options.database.getSpace(canonical.workspaceId) : null;
