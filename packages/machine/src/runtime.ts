...
export async function startMachineRuntime() {
  ...
  if (bootstrapProjectId) {
    const possession = database.possessSpace(
      bootstrapProjectId,
      machineId,
      requiredEnvironment('GITSPACE_BOOTSTRAP_REPOSITORY_PATH'),
    );
    if (possession.status === 'error' && database.getSpacePlacement(bootstrapProjectId)?.holderId !== machineId) {
      throw possession.error;
    }
  }
  if (bootstrapWorkspaceId) {
    const possession = database.possessWorkspace(bootstrapWorkspaceId, machineId);
    if (possession.status === 'error' && database.getWorkspacePossession(bootstrapWorkspaceId)?.holderId !== machineId) {
      throw possession.error;
    }
  }
  ...
  const gitIdentity = new SharedGitIdentityCoordinator(
    authority,
    environmentRoot,
    () => database.listProjects().flatMap((project) => {
      const base = database.getSpace(project.id);
      return base ? [base.rootPath] : [];
    }),
  );
  ...
  const providers = new ProviderAuthCoordinator({ authStorage });
  const omp = new EmbeddedOmpRuntime({
  ...
  const closedSpaceTranscripts = new ClosedSpaceTranscriptReader(authority, encryptedCheckpointBlobs);
  ...
  await devices.start();
  // Self-development: releases are built here from a held workspace, and every generation converges on the tenant's `desired`.
  ...
  const rpc = createGitSpaceRpcHandler({
    ...
    deployment: {
      ...
      thisMachine: { sha: releaseSha, generation: process.env.GITSPACE_GENERATION_HASH ?? null },
    },
  ...
  console.log(`GitSpace RPC ready at ${http.url}/rpc generation=${process.env.GITSPACE_GENERATION_HASH ?? 'unknown'} release=${releaseSha ?? 'local'}`);
