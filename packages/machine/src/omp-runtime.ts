...
export interface OmpRuntimeSession {
  ...
  setThinking(thinking: string | null): OmpSessionControlView;
  setFast(enabled: boolean): OmpSessionControlView;
  setApproval(approvalMode: 'always-ask' | 'write' | 'yolo'): OmpSessionControlView;
  ...
  clearQueue(): OmpSessionControlView;
...
export class EmbeddedOmpRuntime implements OmpRuntime {
  ...
  private async boot(
    ...
    let projectedMcp: ProjectedMcpSession | null = null;
    if (this.options.mcp) {
    ...
    const updateSubagentCount = (): void => {
      const next = AgentRegistry.global().list().filter((ref) => ref.kind === 'sub' && ref.status !== 'aborted').length;
      if (next === subagentCount) return;
      subagentCount = next;
      publishActivity();
    };
    const updateQuestions = (event: OmpRuntimeEvent): boolean => {
      let changed = false;
      if (event.type === 'tool_execution_start' && event.toolName === 'ask') {
        pendingQuestions.add(typeof event.toolCallId === 'string' ? event.toolCallId : 'ask');
        changed = true;
      }
      if (event.type === 'tool_execution_end' && event.toolName === 'ask') {
        pendingQuestions.delete(typeof event.toolCallId === 'string' ? event.toolCallId : 'ask');
        changed = true;
      }
      if (event.type !== 'message_end' || !event.message || typeof event.message !== 'object') return changed;
      const message = event.message as Record<string, unknown>;
      if (message.role === 'assistant' && Array.isArray(message.content)) {
        for (const rawPart of message.content) {
          if (!rawPart || typeof rawPart !== 'object') continue;
          const part = rawPart as Record<string, unknown>;
          if (part.type === 'toolCall' && part.name === 'ask') {
            pendingQuestions.add(typeof part.id === 'string' ? part.id : 'ask');
            changed = true;
          }
        }
      }
      if (message.role === 'toolResult' && message.toolName === 'ask') {
        pendingQuestions.delete(typeof message.toolCallId === 'string' ? message.toolCallId : 'ask');
        changed = true;
      }
      return changed;
    };
    const handleEvent = (event: OmpRuntimeEvent): void => {
      projectedMcp?.recordToolEvent(event);
      const questionChanged = updateQuestions(event);
      switch (event.type) {
        case 'agent_start':
          turnActive = true;
          status = { type: 'busy' };
        ...
        case 'auto_retry_end':
          ...
          break;
        default:
          if (event.type === 'message_end' || event.type === 'tool_execution_end' || questionChanged) publishActivity();
      }
