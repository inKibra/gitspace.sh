...
describe('GitSpaceShell', () => {
  ...
  it('warns in the composer when the selected model provider is not connected on this machine', () => {
    ...
    const sessionControls: NonNullable<GitSpaceShellProps['sessionControls']> = {
      value: { sessionId: 'session-a', role: null, roleLabel: null, roles: [], provider: 'anthropic', models: [{ provider: 'anthropic', id: 'claude', name: 'Claude', contextWindow: null }], model: 'claude', thinking: null, fastMode: false, approvalMode: 'write', context: null, cost: 0, todos: [], queue: { steering: [], followUp: [] }, goal: null, history: [], tree: [] },
      onCycleRole: rejects, onSetModel: rejects, onSetThinking: rejects, onSetFast: rejects, onSetApproval: rejects, onSetGoal: rejects, onCompact: rejects, onClearQueue: rejects, onNavigateTree: rejects,
    };
