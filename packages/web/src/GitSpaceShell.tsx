...
import type { AgentSessionRenderState, SessionControlView } from '@gitspace/protocol';
...
export interface GitSpaceShellProps {
  ...
  mainAgent: { id: string; title: string; state: AgentSessionRenderState; model: string } | null;
  turns: TurnBlock[];
...
function AgentCanvas({ workspace, mainAgent, sessionControls, defaultSendBehavior, turns, transport, onSend, pending, error, onSetWorkspacePhase, onClaimWorkspace, claimMachines = [], homeMachineId = null, defaultMachineId = null, checkpoint = null, providers, banner }: {
  ...
  return <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
    <ScrollArea ref={(element) => { viewport.current = element?.querySelector<HTMLDivElement>('[data-slot=scroll-area-viewport]') ?? null; }} className="min-h-0 flex-1" viewportClassName="h-full">
      <TurnTranscript turns={turns} transport={transport} onAnswer={onSend ? (text) => void onSend(text, 'steer') : undefined} />
    </ScrollArea>
