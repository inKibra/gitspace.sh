...
describe('launchSteps', () => {
  it('follows the interleaved build/upload log per target', () => {
    ...
    expect(steps[1]!.sub).toEqual([{ label: 'Worker', state: 'done' }, { label: 'Machine', state: 'active' }, { label: 'Frontend', state: 'pending' }]);
  });
  ...
  it('keeps Build active through a later upload until every target built', () => {
    ...
    const staged: LaunchTrack = { ...uploading, log: [...uploading.log, { phase: 'build', message: 'building frontend', at: '' }, { phase: 'upload', message: 'uploading 3 frontend files', at: '' }, { phase: 'stage', message: 'staging release', at: '' }] };
  ...
  it('takes the furthest phase when a status poll lands before the event it overtook', () => {
    ...
    const overtaken: LaunchTrack = { ...launchTrackFixture, status: 'succeeded', log: [...launchTrackFixture.log, { phase: 'launched', message: 'worker=skipped frontend=applied', at: '' }, { phase: 'launch', message: 'launching into worker, machine, frontend', at: '' }] };
...
describe('LaunchSheet', () => {
  it('renders the phases with one failed phase, the error, and retry', () => {
    ...
    expect(html).toContain('b7c8d9e0f1 · worker, machine, frontend');
