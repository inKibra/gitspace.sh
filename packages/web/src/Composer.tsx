...
export function Composer({ workspace, controls, providers, skills = [], running, defaultSendBehavior = 'followUp', onSend, pending, error }: ComposerProps) {
  ...
  const [sendBehavior, setSendBehavior] = useState<SendBehavior>(defaultSendBehavior);
  const [showHistory, setShowHistory] = useState(false);

  const submit = async (draft: string, draftAttachments: File[], behavior: SendBehavior = sendBehavior): Promise<void> => {
  ...
  // The server owns the queue: OMP holds steering and follow-up messages and
  // dispatches them itself. The composer mirrors that list into the registry's
  // queue rows; a change that empties the list is the only edit the server
  // exposes (clear), and an appended row is a new follow-up to send.
  const queue = useMemo<QueuedMessage[]>(() => controls
    ? [
      ...controls.value.queue.steering.map((text, index) => ({ id: `steer:${index}:${text}`, text, files: [] })),
      ...controls.value.queue.followUp.map((text, index) => ({ id: `follow:${index}:${text}`, text, files: [] })),
    ]
    : [], [controls]);
  const onQueueChange = (next: QueuedMessage[]): void => {
    if (!controls) return;
    if (next.length === 0 && queue.length > 0) { void controls.onClearQueue(); return; }
  ...
  return <div className="mx-auto flex w-full max-w-xl flex-col gap-2">
    ...
    <InputMessage
      ...
      placeholder={`Ask the ${workspace.kind === 'project' ? 'project' : 'workspace'} agent…`}
      disabled={!onSend || pending}
      files={attachments}
      ...
      rightSlot={controls ? <>
        <Select size="compact" value={sendBehavior} onValueChange={(value) => setSendBehavior(value as SendBehavior)}>
          <SelectTrigger variant="borderless" aria-label="Send behavior" className="max-md:hidden" />
          {options([{ value: 'followUp', label: 'Queue' }, { value: 'steer', label: 'Steer' }])}
        </Select>
        <DropdownMenu size="compact">
          ...
          <DropdownContent align="end" side="top" sideOffset={6} className="min-w-[240px] w-[240px]">
            ...
            <MenuItem index={7} icon={icons['corner-down-left']} label={sendBehavior === 'followUp' ? 'Enter: queue while busy' : 'Enter: steer immediately'} onSelect={() => setSendBehavior((value) => value === 'followUp' ? 'steer' : 'followUp')} closeOnClick={false} className="md:hidden" />
          </DropdownContent>
        </DropdownMenu>
      </> : null}
    />
