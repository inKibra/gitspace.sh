...
export const verticalSliceFixture: GitSpaceShellProps = {
  ...
  turns: [{
    ...
    items: [
      ...
      { id: 'turn-1-tool', type: 'tool-call', toolCallId: 'write-1', tool: 'write', target: 'packages/web/src/GitSpaceShell.tsx', status: 'done', result: [{ id: 'write-result', type: 'code', text: 'Created the new panel shell.', language: 'text' }] },
...
/**
 * One launched release from `workspace-a` (worker applied, frontend applied,
 * the home machine still converging) while a second launch from
 * `workspace-b` is building on this machine.
 */
export const deploymentStatusFixture: DeploymentStatusView = {
  desired: { sha: 'a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0', targets: ['worker', 'machine', 'frontend'], updatedAt: '2026-08-30T10:00:00.000Z' },
  current: {
  ...
  releases: [{
    ...
    status: { worker: 'applied', frontend: 'applied', machines: { studio: 'applied' } },
    error: null,
  }],
