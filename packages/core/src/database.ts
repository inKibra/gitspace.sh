...
import { z } from 'zod';
import { assertWorkspacePhase, emptyRelations, resolveWorkspaceRelations, WorkspaceDomainError, WorkspacePhaseSchema, type WorkspaceRelations, type WorkspaceRelationsInput } from '@gitspace/protocol-workspace';
import {
  artifactScopes,
  factEvents,
  projects,
...
export class GitSpaceDatabase {
  ...
  setWorkspacePhase(workspaceId: string, phase: Workspace['phase']): Workspace | null {
    const parsed = WorkspacePhaseSchema.parse(phase);
    const current = this.getWorkspace(workspaceId);
    if (!current) return null;
    const dependencies = this.getSpaceRelations(workspaceId).dependsOn
      .map((id) => this.getWorkspace(id)).filter((dependency): dependency is Workspace => dependency !== null);
    assertWorkspacePhase(parsed, dependencies);
    const now = new Date().toISOString();
    this.orm.transaction((tx) => {
      tx.update(spaces).set({ phase: parsed, updatedAt: now }).where(eq(spaces.id, workspaceId)).run();
      tx.insert(factEvents).values({ projectId: current.projectId, scope: 'workspace', entity: 'workspace', entityId: workspaceId, revision: sql`(SELECT COALESCE(MAX(revision), 0) + 1 FROM fact_events WHERE project_id = ${current.projectId} AND entity_id = ${workspaceId})`, operation: 'updated', payload: { phase: parsed }, createdAt: now }).run();
    });
  ...
  setSpaceRelations(workspaceId: string, input: WorkspaceRelationsInput): ResultType<WorkspaceRelations, WorkspaceDomainError> {
    const workspace = this.getWorkspace(workspaceId);
    let relations: WorkspaceRelations;
    try {
      relations = resolveWorkspaceRelations(workspaceId, input,
        workspace ? this.listWorkspaces(workspace.projectId) : [],
        workspace ? this.listSpaceRelations(workspace.projectId) : new Map());
    } catch (error) {
      if (error instanceof WorkspaceDomainError) return Result.err(error);
      throw error;
    }
    const now = new Date().toISOString();
    this.orm.transaction((tx) => {
      ...
      tx.update(spaces).set({ updatedAt: now }).where(eq(spaces.id, workspaceId)).run();
      tx.insert(factEvents).values({ projectId: workspace!.projectId, scope: 'workspace', entity: 'workspace', entityId: workspaceId, revision: sql`(SELECT COALESCE(MAX(revision), 0) + 1 FROM fact_events WHERE project_id = ${workspace!.projectId} AND entity_id = ${workspaceId})`, operation: 'updated', payload: { relations }, createdAt: now }).run();
    });
  ...
  /** Caller must verify the same open authoritative lease and an intact checkout.
   * Restoring this fenced cache neither acquires a lease nor advances its generation. */
  adoptOpenSpaceProjection(input: { spaceId: string; holderId: string; expectedGeneration: number; rootPath: string }): ResultType<SpacePlacement, CoreConflict> {
    if (!idSchema.safeParse(input.holderId).success || input.holderId === 'unassigned'
      || !Number.isSafeInteger(input.expectedGeneration) || input.expectedGeneration <= 0) {
      return Result.err(conflict('possession', input.spaceId, 'Open space projection requires a valid holder and authority generation'));
    }
    const changed = this.orm.update(spacePlacements).set({
      ...
      eq(spacePlacements.holderId, 'unassigned'),
      eq(spacePlacements.generation, input.expectedGeneration),
      eq(spacePlacements.rootPath, input.rootPath),
      eq(spacePlacements.state, 'closed'),
    )).returning().get();
    return changed ? Result.ok(changed) : Result.err(conflict('possession', input.spaceId, 'Space placement changed before adopting the open authority projection'));
  }
