...
Approval permits repository code to run as the machine user, with access to that user's home configuration and available credentials. It is not a sandbox. Secret grants control GitSpace-injected secrets, not every credential a script can find on the machine. Hashing an entry script does not pin every helper, package install hook, executable, or remote payload it invokes. Review those dependencies and their effects too. Log redaction does not prevent a script from reading other files or sending data over the network.
...
| `GITSPACE_MACHINE_TOOLS` | Private, stable machine tool directory; its `bin` directory takes precedence over tools on the inherited `PATH` |
...
Scripts run on an authorized machine runner, not inside a Cloudflare Worker. They inherit the machine user's `HOME`, `PATH`, and ordinary environment. GitSpace prepends `GITSPACE_MACHINE_TOOLS/bin` to `PATH`; declared values and granted secrets override inherited values. Internal `GITSPACE_*` daemon bootstrap variables are excluded, and the runner supplies its own workspace metadata. Git uses the user's normal configuration, with interactive prompting disabled.

Each run keeps its logs and output in a separate temporary directory. Cleanup removes that directory, not the user's home. Use checks to verify existing tool versions and `machine/prepare` to install missing tools. Inheritance uses the daemon's environment; it does not search for tools outside its `PATH` or source login dotfiles. A cloud machine uses its own home, not a copy of your personal machine.
...
Each script result has its own `exitCode`: zero means success, a nonzero value means failure, and null means no exit was recorded. A started script with no exit is running while the phase is active and interrupted after the phase ends. A script with no result has not started. Available `startedAt` and `finishedAt` timestamps describe that script, not the whole phase.

Full run output lives in durable cloud log chunks, not in an artifact file inside the checkout. The wire `environment.runLog` request accepts `spaceId`, `runId`, and nullable `offset`. It returns `output`, `nextOffset`, and `cursor`. Follow `nextOffset` while more stored chunks are available. A null `nextOffset` means caught up for now, not necessarily that a running script has finished; retain `cursor`, the first unread stored row, and use it as the next request's offset after a run update. The agent wrapper uses its discovered workspace-target schema. Read subsequent chunks rather than treating the preview as the full log.
