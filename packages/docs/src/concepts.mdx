...
The account selects cloud images independently for each machine, plus a default for new machines. The platform supplies provider infrastructure; changing one machine's selected image does not update the others. An image supplies the operating system, tools, and initial runtime, so replacing it is separate from updating account code inside the running VM. Image replacement checkpoints the source, confirms it has stopped, and keeps admission closed until the replacement restores the previously open workspaces. Stopping a Cloudflare VM discards its ephemeral disk; status and checkpoint-control requests must not silently start a fresh VM.
...
Chat opens at the newest messages and loads older blocks as you scroll back. It mounts the visible blocks and a small margin, then evicts distant history as the session grows. The history and complete-content caches each have a 64 MiB background budget and a 384-entry limit. Visible content can exceed those budgets; they do not limit what you can read. Eviction removes browser copies, not saved history.

Messages, tool results, questions, and attachments render in full through the existing transcript controls. Large blocks load automatically as they approach the viewport, without a separate content viewer. New output does not move your reading position. Use **Jump to latest** to resume following it. Full transcript reads remain lossless.

Prompt recall keeps a small set of recent, complete prompts. The history browser loads older prompts and branch entries on demand rather than keeping the whole session in memory.
...
An open portable workspace has its own Git repository. Inspector can browse its files and staged or working changes without the project's base checkout.

Base comparisons use the original branch commit from the base space's latest published checkpoint. Inspector fetches any missing Git objects into the inspected workspace and compares changes from the merge base. It does not reopen the base space or include the base space's saved uncommitted changes. If the checkpoint is unavailable, the base comparison reports that error; files and local changes remain available.
