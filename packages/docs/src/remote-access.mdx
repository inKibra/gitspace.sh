...
Routine requests check device permissions and account and tenant access state. They do not read GitSpace platform credit balances. Access restrictions still apply, including suspension and quarantine. Requests fail closed when the account or tenant access authority is unavailable.

GitSpace records worker-request usage in the background on a best-effort basis. A billing outage does not block ordinary requests, but it can leave usage unrecorded. [Issue #147](https://github.com/inKibra/gitspace.sh/issues/147) tracks durable usage delivery, automatic overage enforcement, and shutdown policy.
