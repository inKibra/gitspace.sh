...
describe('signed control transport', () => {
  it('routes a replay-protected device-signed request to the canonical space authority', async () => {
    ...
    await env.ACCOUNTS.getByName('global').upsertProvisioning({ userId, handle: `control-${crypto.randomUUID().slice(0, 8)}` });
  ...
  it('stores signed application data in the dedicated R2 binding', async () => {
    ...
    const vault = env.CREDENTIALS.getByName(userId);
    await env.ACCOUNTS.getByName('global').upsertProvisioning({ userId, handle: `data-${crypto.randomUUID().slice(0, 8)}` });
    await env.ACCOUNTS.getByName('global').markActive({ userId, release: null });
    await vault.bootstrap({
      userId,
      rootPublicKey: credentialProtocolBase64.encode(ed25519.getPublicKey(rootPrivateKey)),
  ...
  it('authorizes canonical settings writes and reports stale generations', async () => {
    ...
    const vault = env.CREDENTIALS.getByName(userId);
    await env.ACCOUNTS.getByName('global').upsertProvisioning({ userId, handle: `settings-${crypto.randomUUID().slice(0, 8)}` });
    await env.ACCOUNTS.getByName('global').markActive({ userId, release: null });
    await vault.bootstrap({
      userId,
      rootPublicKey: credentialProtocolBase64.encode(ed25519.getPublicKey(rootPrivateKey)),
  ...
  it('pushes settings generations to every authenticated machine subscriber', async () => {
    ...
    const vault = env.CREDENTIALS.getByName(userId);
    await env.ACCOUNTS.getByName('global').upsertProvisioning({ userId, handle: `stream-${crypto.randomUUID().slice(0, 8)}` });
    await env.ACCOUNTS.getByName('global').markActive({ userId, release: null });
    await vault.bootstrap({
      userId,
      rootPublicKey: credentialProtocolBase64.encode(ed25519.getPublicKey(rootPrivateKey)),
