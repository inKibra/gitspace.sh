...
async function bootstrapUser(): Promise<string> {
  ...
  await env.ACCOUNTS.getByName('global').upsertProvisioning({ userId, handle: `device-${crypto.randomUUID().slice(0, 8)}` });
...
describe('account provider credentials', () => {
  it('round-trips, rotates, and deletes a write-only Composio key', async () => {
    const userId = await bootstrapUser();
    const vault = env.CREDENTIALS.getByName(userId);
    const firstKey = 'composio-test-key-first-value';
    const secondKey = 'composio-test-key-second-value';

    const first = await vault.putProviderSecret('composio', firstKey);
    expect(first).toMatchObject({ configured: true, revision: 1 });
    expect(await vault.providerSecretMetadata('composio')).toMatchObject({ configured: true, revision: 1 });
    expect(await vault.getProviderSecret('composio')).toBe(firstKey);

    const second = await vault.putProviderSecret('composio', secondKey);
    expect(second.revision).toBe(2);
    expect(await vault.getProviderSecret('composio')).toBe(secondKey);

    expect(await vault.deleteProviderSecret('composio')).toEqual({ deleted: true });
    expect(await vault.getProviderSecret('composio')).toBeNull();
    expect(await vault.providerSecretMetadata('composio')).toEqual({ configured: false, revision: 0, updatedAt: null });
  });
});
