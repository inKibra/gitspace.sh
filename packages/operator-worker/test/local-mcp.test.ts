...
describe('ComposioPluginGateway', () => {
  it('reports an honest unavailable catalog and refuses authorization without a Worker secret', async () => {
    const gateway = new ComposioPluginGateway({} as Env);
    await expect(gateway.catalog()).resolves.toEqual({ configured: false, toolkits: [] });
    await expect(gateway.authorize('principal-a', 'github', 'https://gitspace.test/callback')).rejects.toThrow();
  });
});
...
describe('UserMcpConnectionsDO', () => {
  ...
  it('stores Composio plugins without credentials and consumes authorization state once', async () => {
    const stub = mcpEnv.USER_MCP_CONNECTIONS.getByName('mcp-composio');
    const created = await runInDurableObject(stub, (authority: UserMcpConnectionsDO) => authority.createComposio('principal-a', {
