...
export class FleetCatalogDO extends DurableObject<Env> {
  ...
  putMachine(input: FleetMachineDefinition): FleetMachineDefinition {
    ...
    this.broadcast({ type: 'upsert', machineId: input.id, machine: input });
    return input;
  }
  ...
  removeMachine(machineId: string): boolean {
    ...
    if (removed) this.broadcast({ type: 'remove', machineId, machine: null });
    return removed;
  }
  ...
  private broadcast(event: { type: 'upsert' | 'remove'; machineId: string; machine: FleetMachineDefinition | null }): void {
