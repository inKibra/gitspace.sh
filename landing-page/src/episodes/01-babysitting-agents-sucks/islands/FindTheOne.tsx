...
const WRONG: Record<State, string> = { active: "still working…", closed: "you closed this", amber: "escalation", idle: "" };
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden">
            ...
            return (
              <li key={w.id}>
                <button
                  onClick={() => clickRow(w)}
                  disabled={found}
                  className={cn("w-full text-left px-4 py-3 flex items-center gap-3 transition-colors", !found && "hover:bg-zinc-900/60", hit && "bg-blue-500/10")}
                >
                  {w.state === "active" ? (
                  ...
                  {hit && (
                    <span className="text-[11px] text-blue-400 flex items-center gap-1 shrink-0">
                      <Check className="w-3 h-3" /> idle — your move
                    </span>
                  )}
        ...
        <div className="px-4 py-3 border-t border-zinc-800 text-[12px] text-zinc-500 flex items-start gap-2">
          ...
          {found ? (
            ...
            <span>
              <span className="text-zinc-300">Three agents have stopped spinning.</span> One is idle, waiting on your next move — the others you closed or
              are escalating. Click the idle one.
            </span>
          )}
        </div>
      </div>
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden relative">
        <div className="px-4 py-2.5 border-b border-zinc-800 flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {FLEET.map((w) => (
              <div
                key={w.id}
                className={cn(
                  "flex items-center gap-2 px-2.5 py-1.5 rounded text-sm font-mono",
                  w.state === "idle" ? "bg-blue-500/10 ring-1 ring-blue-400/50" : "bg-zinc-900/60",
                  w.state === "closed" && "opacity-40"
                )}
              >
                <span className={cn("h-2 w-2 rounded-full", DOT[w.state], w.state === "active" && "shadow-[0_0_6px] shadow-green-500/60")} />
        ...
        {!revealed && (
          <button onClick={() => setRevealed(true)} className="absolute inset-x-0 bottom-12 top-[41px] flex items-center justify-center">
            <span className="px-4 py-2 rounded-md bg-green-500 text-black text-sm font-medium shadow-lg">Reveal the bar →</span>
          </button>
        )}
        <div className="px-4 py-3 border-t border-zinc-800 text-[12px]">
          {revealed ? (
            <span className="text-blue-400">The idle one is blue — your move. Working, escalating, and closed each get their own color too.</span>
          ) : (
