  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
        <span className="text-sm text-zinc-300 font-medium">Reading vs. seeing</span>
      ...
      <div className="relative border-t border-zinc-800">
        <div className={cn("h-[280px] overflow-y-auto divide-y divide-zinc-900", !armed && "overflow-hidden pointer-events-none")}>
          {list.map((r) => (
            <button key={r.name} onClick={() => pick(r)} className="w-full text-left px-4 py-2.5 flex items-center gap-3 hover:bg-zinc-900/60 transition-colors">
              <span className="w-12 shrink-0 flex items-center">
                {mode === "color" ? (
                  <span className={cn("h-2.5 w-2.5 rounded-full", r.idle ? "bg-blue-500" : "bg-green-500")} />
                ) : (
        ...
        {!armed && (
          <div className="absolute inset-0 flex items-center justify-center backdrop-blur-md bg-zinc-950/55 p-4 text-center">
            {phase === "introW" && (
              <button onClick={startWords} className="group flex flex-col items-center gap-2">
                <span className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-white/10 bg-white/10 text-zinc-100 text-sm font-medium shadow-lg group-hover:bg-white/[0.16] transition-colors">
            ...
            {phase === "introC" && (
              <button onClick={startColor} className="group flex flex-col items-center gap-3 max-w-sm">
                ...
                <span className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-blue-400/30 bg-blue-500/15 text-blue-200 text-sm font-medium shadow-lg group-hover:bg-blue-500/25 transition-colors">
            ...
            {phase === "done" && (
              <div className="flex flex-col items-center gap-3 max-w-md">
                ...
                <p className="text-[13px] text-zinc-400 leading-relaxed">
                  Sixteen rows and you already feel it. A real fleet is a sidebar you scroll forever — reading every name doesn’t keep up. Color does.
