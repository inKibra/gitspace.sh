...
const SAMPLES: { label: string; block: unknown }[] = [
  ...
  { label: 'rule-activation (project rule matched a call)', block: { id: '17b', type: 'rule-activation', data: { rule: 'ts-no-tiny-functions', reason: 'rule_violation', path: 'builtin-defaults:ts-no-tiny-functions.md', body: 'Do not extract a function whose whole body is one expression or one `return`. Inline it unless the name creates a durable contract.\n\n## Why\n\n- One-line wrappers hide no real behavior.\n- Readers must jump to verify trivial code.' } } },
  { label: 'rule-activation · interrupted (aborted mid-stream, retried)', block: { id: '17c', type: 'rule-activation', data: { rule: 'ts-no-return-type', reason: 'rule_violation', path: 'builtin-defaults:ts-no-return-type.md', interrupted: true, body: 'Your output was interrupted: it published a contract derived from a function type instead of naming it.\n\nName the type at the module that owns the value, and import that name at consumers.' } } },
  { label: 'recap (idle — transient, never persisted)', block: { id: '17d', type: 'recap', data: { text: 'Goal: get the release branch merge-ready. Currently making a lit chain dot always mean workspace status. Next: check the kanban stacks lens in the browser.' } } },
