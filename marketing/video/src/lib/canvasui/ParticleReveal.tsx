/**
 * VENDORED from Canvas UI (canvasui.dev) — `particle-reveal-react`.
 * License: MIT + Commons Clause (we are not selling the software).
 *
 * Patched for Remotion: the upstream effect animates itself with
 * requestAnimationFrame + live pointer events, which is non-deterministic and
 * therefore unusable in a frame-by-frame renderer. `render()` upstream is
 * already a pure function of (pointer, time, config), so the patch adds a
 * `manual` mode that suppresses the rAF loop and a `renderFrame()` entry
 * point that sets that state explicitly and draws exactly once.
 */
"use client";
...
export interface ParticleRevealInstance {
  ...
  /** Remotion: draw one deterministic frame. */
  renderFrame: (state: { x: number; y: number; active: number; time: number }) => void;
...
type UniformName =
  | 'uContent' | 'uRes' | 'uDpr' | 'uPointer' | 'uActive' | 'uRadius'
  | 'uSoftness' | 'uSize' | 'uScatter' | 'uDrift' | 'uAberration' | 'uBend'
  | 'uFade' | 'uThreshold' | 'uBg' | 'uTime' | 'uMaxX' | 'uCrisp';
...
export function createParticleReveal(
  ...
  // NOTE: this project enables `noUncheckedIndexedAccess`, which widens index
  // signatures to `| undefined`. A mapped type over the known uniform names has
  // fixed keys, so upstream's dot access keeps working unchanged.
  ...
  return {
    renderFrame(state) {
      // recapture the (animating) DOM, then set pointer/time and draw once
      if (htmlInCanvas) {
        try {
          sourceCtx!.reset();
          sourceCtx!.drawElementImage!(content, 0, 0);
      ...
      pointer.x = pointer.tx = state.x;
      pointer.y = pointer.ty = state.y;
      pointer.active = pointer.target = state.active;
      time = state.time;
      render();
    },
    setOptions(next) {
