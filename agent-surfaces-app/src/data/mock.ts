  ...
  phases: [
    {
      ...
      created: [
        { name: 'types brief', type: 'goal-slice', from: 'goal.md §Data model · L12–28', passedTo: 'task' },
        { name: 'type-review rubric', type: 'rubric', from: 'authored for this phase', passedTo: 'reviewer' },
      ],
      nodes: [
        { id: 'i1', agent: 'task', kind: 'agent', status: 'done', reads: [{ name: 'types brief', io: 'artifact' }, { name: 'effects/a.ts', io: 'source' }], writes: [{ name: 'types.ts', io: 'source' }], out: 'draft types.ts' },
        { id: 'r1', agent: 'reviewer', kind: 'agent', status: 'done', reads: [{ name: 'type-review rubric', io: 'artifact' }, { name: 'types.ts', io: 'source' }], writes: [{ name: 'type-review.md', io: 'artifact' }], out: 'verdict + notes' },
    ...
    },
    {
      ...
      created: [{ name: 'core rubric', type: 'rubric', from: 'authored for this phase', passedTo: 'reviewer' }],
      nodes: [
        { id: 'i2', agent: 'task', kind: 'agent', status: 'done', reads: [{ name: 'types.ts', io: 'source' }], writes: [{ name: 'core.ts', io: 'source' }, { name: 'core.test.ts', io: 'source' }], out: 'core.ts + tests' },
        ...
        { id: 'r2', agent: 'reviewer', kind: 'agent', status: 'running', reads: [{ name: 'core rubric', io: 'artifact' }, { name: 'core.ts', io: 'source' }, { name: 'test-results', io: 'artifact' }], writes: [{ name: 'core-review.md', io: 'artifact' }], out: 'verdict' },
    ...
    },
    {
      ...
      created: [
        { name: 'migration plan', type: 'phased-goal', from: 'goal.md §Implementation plan · step 4', passedTo: 'task' },
        { name: 'visual-diff rubric', type: 'rubric', passedTo: 'reviewer' },
      ],
      nodes: [
        { id: 'i3', agent: 'task', kind: 'agent', status: 'pending', reads: [{ name: 'migration plan', io: 'artifact' }, { name: 'core.ts', io: 'source' }], writes: [{ name: 'pipeline.ts', io: 'source' }, { name: 'Editor.tsx', io: 'source' }], out: 'migrated files' },
        { id: 'r3', agent: 'reviewer', modelRole: 'vision', kind: 'agent', status: 'pending', reads: [{ name: 'visual-diff rubric', io: 'artifact' }, { name: 'diff', io: 'source' }], writes: [{ name: 'migrate-review.md', io: 'artifact' }], out: 'verdict', fanout: { over: 'each changed consumer', instances: ['pipeline.ts', 'Editor.tsx', 'Scramble.ts'] } },
